...
from djangae.contrib.lock.lock import lock, LockAcquisitionError
...
class LocksTestCase(TestCase):
    ...
    def test_context_manager_no_wait(self):
        ...
        my_lock = Lock.objects.acquire('x')
    ...
    def test_cleanup_view(self):
        ...
        Lock.objects.create(identifier="old_lock", timestamp=ages_ago)
        recent_lock = Lock.objects.create(identifier="recent_lock")
        cleanup_locks(None)
        self.process_task_queues()
        # The old lock should have been deleted but the new one should not
        self.assertItemsEqual(Lock.objects.all(), [recent_lock])
