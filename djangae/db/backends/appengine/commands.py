...
class SelectCommand(object):
    ...
    def _build_query(self):
        ...
        if len(queries) == 1:
            ...
            if identifier:
                # Yay for optimizations!
                return meta_queries.UniqueQuery(self.connection, identifier, queries[0], self.query.model, self.namespace)
        ...
        if self.query.kind == "COUNT":
            if excluded_pks:
                ...
                if isinstance(query, meta_queries.QueryByKeys):
                    ...
                    count_query = self.connection.query(query._Query__kind, keys_only=True, namespace=self.namespace)
...
class FlushCommand(object):
    ...
    def execute(self):
        ...
        while query.Count():
            self.connection.delete_multi(query.Run())
        ...
        def txn():
            ...
            if (
                not isinstance(
                    ...
                    (meta_queries.QueryByKeys, meta_queries.NoOpQuery)
                ) # ignore QueryByKeys and NoOpQuery
